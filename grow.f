...
!!    on the plant这个子程序根据水分、温度和养分胁迫对植物的影响来调整植物生物量、叶面积指数和冠层高度
...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      rto = 1.
        ...
        !! plant will not undergo stress if dormant植物在休眠时不会受到压力
        if (idorm(j) == 1) return
        idp = idplt(j)
		
        !! update accumulated heat units for the plant更新工厂的累计热量单位
        delg = 0.
        ...
        !! if plant hasn't reached maturity如果植物还没有成熟
        if (phuacc(j) <= 1.) then

         !! compute temperature stress - strstmp(j)  计算机温度应力 
          call tstr
     ...
     &          (laiday(j) + .05)))
          ...
          !! adjust radiation-use efficiency for CO2调整二氧化碳的辐射利用效率
          beadj = 0.
          if (co2(hru_sub(j)) > 330.) then
            beadj = 100. * co2(hru_sub(j)) / (co2(hru_sub(j)) +      !式子5-12   
     &              Exp(wac21(idp) - co2(hru_sub(j)) * wac22(idp)))     
          ...
          !!assumes vapor pressure threshold of 1.0 kPa假设蒸气压闯值为1.0 kPa，则调整蒸气压赤字的辐射使用效率
          if (vpd > 1.0) then
            ruedecl = 0.
            ruedecl = vpd - 1.0!式子5-15
		...
		!! 计算植株氮磷吸收量随cibin 02/15/12的变化，以确保在温度、水分和通气胁迫下植株不吸收氮磷。
          reg = 0.
          reg = Min(strsw(j), strstmp(j), strsa(j))
          if (reg < 0.) reg = 0.

	    if (reg > 0.) then
            call nup!这个子程序计算植物的氮吸收量。
            call npup!这个子程序计算植物对磷的吸收。
	    else
	      strsn(j) = 1.
	      strsp(j) = 1.
	    end if

          !! auto fertilization-nitrogen demand (non-legumes only)自动施肥-需氮量(仅限非豆类)
          select case (idc(idp))
            case (4, 5, 6, 7)
            if (auto_nstrs(j) > 0.) call anfert!当氮胁迫超过用户输入闽值时，该子程序自动应用氮和磷。
          end select

          !! reduce predicted biomass due to stress on plant由于对植物的压力而减少预测生物量
          reg = 0.
     ...
     &                                         bio_targ(j)!5-88
          ...
          bio_ms(j) = bio_ms(j) + bioday * reg!5-86
          ...
          !! calculate fraction of total biomass that is in the roots计算根中总生物量的分数
     ...
     &                     - leaf2(idp) * phuacc(j)))
          ...
          !! calculate new canopy height计算新冠高度
          if (idc(idp) == 7) then
            cht(j) = rto * chtmx(idp)
          else
          ...
          !! calculate new leaf area index计算新叶面积指数
