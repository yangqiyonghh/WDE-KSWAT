...
!! compute bacteria die-off and re-growth on foilage
      if (tmpav(j) > 1.e-6) then
        bactp_plt(j) = bactp_plt(j) * Exp(-Theta(wp20p_plt,1.07,
     &    tmpav(j))) - 0.0
        bactp_plt(j) = Max(0., bactp_plt(j))
        if (bactp_plt(j) < 0.0) bactp_plt(j) = 0.
        bactlp_plt(j) = bactlp_plt(j) * Exp(-Theta(wp20lp_plt,1.07,
     &    tmpav(j))) - 0.0
        bactlp_plt(j) = Max(0., bactlp_plt(j))
        if (bactlp_plt(j) < 0.0) bactlp_plt(j) = 0.
      endif
...
!! compute bacteria die-off and re-growth in surface soil layer
      ...
      blps = 0.
      bpq = bactpq(j)
      bactpq(j) = bactpq(j) * Exp(-Theta(wpq20,1.07,tmpav(j))) -
     &    0.0
      bactpq(j) = Max(0., bactpq(j))
      if (bactpq(j) < 0.0) bactpq(j) = 0.
      blpq = bactlpq(j) 
      bactlpq(j) = bactlpq(j) * Exp(-Theta(wlpq20,1.07,tmpav(j))) -
     &    0.0
      bactlpq(j) = Max(0., bactlpq(j))
      if (bactlpq(j) < 0.0) bactlpq(j) = 0.
      bps = bactps(j)
      bactps(j) = bactps(j) * Exp(-Theta(wps20,1.07,tmpav(j))) -
     &    0.0
      bactps(j) = Max(0., bactps(j))
      if (bactps(j) < 0.0) bactps(j) = 0.
      blps = bactlps(j)
      bactlps(j) = bactlps(j) * Exp(-Theta(wlps20,1.07,tmpav(j))) -
     &    0.0
      bactlps(j) = Max(0., bactlps(j))
      if (bactlps(j) < 0.0) bactlps(j) = 0.

!! compute bacteria in the runoff
      bactrop = bactpq(j) * surfq(j) /                                  
     &                        (sol_bd(1,j) * sol_z(1,j) * 75.)
