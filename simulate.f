...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      if (station_flag==1)then
          allocate(karst_parm(num))!存放已率定好的上游参数
          do k=1,num
      ...
      alpha_delay1=sftmp!(0.1-5)下渗给表层岩溶带的延迟系数，就是比如土壤下渗量并不是一下子就补给到岩溶带中，存在滞时，可分干湿期间
      alpha_delay2=smtmp!(0.1-5)表层岩溶带传递给基质的过程，,存在滞时，可分干湿期间
      sur_karst_h1=smfmn!(1-50)表层岩溶带发生延迟流的阈值水深
      sur_karst_h2=smfmn+smfmx!(1-20)表层岩溶带发生二次流的阈值水深
      k_sec=timp!(0.0001-0.5)表层岩溶带二次流的出流比流量系数(线性出流)，可分干湿期间
      k_eh=snocovmx!(0.0001-0.5)表层岩溶带延迟出流比流量系数，可分干湿期间
      alpha_eh=sno50cov!(0.5-2)表层岩溶带延迟出流指数系数，可分干湿期间
      k_f=spcon_bsn!(0.0001-0.5)表层岩溶带传递给管道流，这里与基质流传递方式不一样，线性传递，可分干湿期间
      gw_karst_delaye=spexp_bsn!(0.1-5)补给地下岩溶基质的延迟系数，可分干湿期间
      gw_alpha_bfe=n_updis!(0.1-5)地下岩溶基质出流的延迟系数，可分干湿期间
      k_fm=p_updis!(0.0001-0.5)管道流的出流系数，可分干湿期间
      alpha_FM=pperco_bsn!(0.5-3)管道流的比流量指数系数?，可分干湿期间
      k_sc=phoskd_bsn!(0.0001-0.5)基质和管道的交换系数，可分干湿期间
      alpha_sc=psp_bsn!(0.1-1)基质和管道的交换指数，可分干湿期间
      ...
      sub_karst=1 !1:刁江流域,0:滂江流域
      cn_flag=0!修改CN
      call gw_karst_num   !地下河所在子流域的编号
      eof = 0
