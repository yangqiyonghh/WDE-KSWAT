...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      if (cn_flag==1) cnday(j) = cnday(j) *(15.63*hru_slp(j)+322.79)/(hru_slp(j)+323.52)
      if (api_idum==1)then
          cnday(j) =(2.71**IWAPI_1(j))* cnday(j) 
          cnday(j)=max(cnday(j),35.0)
      endif
      
      
      cnday(j)=min(cnday(j),98.0)
      sol_cnsw(j) = sol_sw(j)
      !if (j==1)print*,i,kkk,cnday(j),r2 
      return
      end