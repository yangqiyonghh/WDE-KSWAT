...
!! sum HRU results for subbasin 
      ...
      !! subbasin averages: water
        ...
        sub_karst_SEC_EH(sb) = sub_karst_SEC_EH(sb) +  (Q_sur_sec(j)+ Q_sur_eh(j))* hru_fr(j)
        sub_karst_FM(sb) = sub_karst_FM(sb) + Q_karst_FM(j) * hru_fr(j)
          epikarst_recharge(sb)=epikarst_recharge(sb)+sur_rchrg_karst(j)* hru_fr(j)
          Aquifer_recharge(sb)=Aquifer_recharge(sb)+Q_karst_S1(j)* hru_fr(j)
          
          Conduct_recharge(sb)=Conduct_recharge(sb)+Q_karst_F1(j)* hru_fr(j)
          if (gw_karst_hru(inum1)==1)then
          Matrix_conduct_flow_exchange(sb)=Matrix_conduct_flow_exchange(sb)  +karst_sc1(j)* hru_fr(j)
          endif

        sub_sec(sb)=sub_sec(sb)+Q_sur_sec(j)* hru_fr(j)!表层岩溶带二次流
        sub_EH(sb)=sub_EH(sb)+Q_sur_eh(j)* hru_fr(j)!表层岩溶带裂隙流
        sub_FM(sb)=sub_FM(sb)+Q_karst_FM(j)* hru_fr(j)!管道出流
...
!! perform subbasin level operations after processing last HRU in subbasin
      if (iihru == hrutot(sb)) then
          open(777,file="karst_flow.txt")
          write(777,5111)sb,iyr,i,sub_karst_SEC_EH(sb),sub_karst_FM(sb),epikarst_recharge(sb),Aquifer_recharge(sb),
     &     Conduct_recharge(sb),Matrix_conduct_flow_exchange(sb)
 5111 format ('SUB ',i3,3x,i4,3x,i4,10x,e12.4,10x,e12.4,10x,e12.4,10x,e12.4,10x,e12.4,10x,e12.4,10x,e12.4)          
        sub_ha = 0.
