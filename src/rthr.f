...
!!                                   |main channel alluvium主河道冲积层的有效导水率
...
!!    rnum1           |none          |fraction of overland flow地表径流的比例
!!    ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
...
!!    hrttlc(:)    |m^3 H2O       |transmission losses for hour小时传输损耗
...
!!     start of sub-daily loop
      ...
      do ii = 1, nstep
        ...
        !! water entering reach during time step时间步长期间进水范围
        ...
        !! calculate cross-sectional area of flow计算流量的横截面积

        hharea(ii) = vol / (ch_l2(jrch) * 1000.)

        !! calculate depth of flow计算水流深度
     ...
     &     ch_w(2,jrch) + 2. * (hdepth(ii) - ch_d(jrch)) * Sqrt(17.)
        ...
        if (hsdti(ii) > 0.) then
          ...
          !! calculate volume of water leaving reach on day计算当天离开河段的水量
          ...
          !! calculate transmission losses计算传输损耗
          ...
          if (hrtwtr(ii) > 0.) then
            ...
            !! calculate width of channel at water level计算水位处的渠道宽度
     ...
     &          hhtime(ii)
          ...
          !! set volume of water in channel at end of hour设置小时结束时通道中的水量
...
!! calculate amount of water in channel at end of day计算一天结束时河道中的水量
...
!! calculation of daily average values日平均值的计算

      !! set volume of water in reach at end of day一天结束时可达到的设定水量
      rchstor(jrch) = hhstor(nstep)
      !! calculate total amount of water leaving reach计算离开河段的总水量
      rtwtr = Sum(hrtwtr)
      !! calculate average flow area计算平均流动面积
      rcharea = Sum (hharea) / nstep
      !! calculate average flow depth计算平均流深
      rchdep = Sum(hdepth) / nstep
      !! calculate average flow rate计算平均流量
