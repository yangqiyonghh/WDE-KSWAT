...
!!    LID general
	...
	allocate(rspc_a(mhru))	
      ...
      allocate(sur_rchrg_karst(mhru))!进入表层岩溶带中的补给水量
      allocate(sur_karst_height(mhru))!表层岩溶带的储水量
      allocate(Q_sur_sec(mhru))!表层岩溶带极端二次流出流流量
      allocate(Q_sur_eh(mhru))!表层岩溶带延迟出流
      allocate(Q_karst_S(mhru))!表层岩溶带传递给基质
      allocate(Q_karst_F(mhru))!表层岩溶带传递给地下管道
      allocate(gw_karst_hru(msub))!子流域是否存在地下管道，1是存在，0是不存在
      allocate(karst_hru(mhru))!如果是岩溶子流域就是1，非岩溶hru就是0
      allocate(Q_karst_FM(mhru))!地下河出流
      allocate(karst_FM_height(mhru))!地下河储水量
      allocate(karst_sc(mhru))
      allocate(sub_sec(msub))!表层岩溶带二次出流
      allocate(sub_EH(msub))!表层岩溶带裂隙出流
      allocate(sub_FM(msub))!管道出流
      allocate(karst_type(mhru))!喀斯特的种类，0代表非岩溶，1代表碳酸盐岩裂隙溶洞水：石灰岩裂隙溶洞水，2代表碳酸盐岩夹碎屑岩裂隙溶洞水，3代表碳酸盐岩裂隙溶洞水：白云岩裂隙孔洞裂隙水
      allocate(pre_pcp(mhru,200))!存放之前的降雨数据
      allocate(IWAPI(mhru))
      allocate(WAPI(mhru))  
      allocate(IWAPI_1(mhru))
      allocate(WAPI_1(mhru))        
      allocate(wapi_all(mhru))
      allocate(wapi_all_1(mhru))
      allocate(origin_sol_k_parm(mlyr,mhru))
      allocate(origin_gw1_parm(mhru))
      allocate(origin_gw2_parm(mhru))    
      allocate(origin_esco_parm(mhru)) 
      allocate(origin_brt_parm(mhru)) 
      allocate(origin_ch_n(msub))
      allocate(origin_ch_k(msub))
      allocate(sub_karst_SEC_EH(msub))
      allocate(sub_karst_FM(msub))
      allocate(epikarst_recharge(msub))
