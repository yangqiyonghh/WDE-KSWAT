...
!     skipping hourly bacteria route for now  04/16/07 nubs
     ...
     &                               (1. - rnum1) + initlp * hrchwtr(ii)
          ...
          totbactp = totbactp * Exp(-Theta(0.0 / 24.,1.07,wtmp))
          totbactp = Max(0., totbactp)
          totbactlp = totbactlp * Exp(-Theta(0.0 / 24.,1.07,wtmp))
...
!! daily mass balance
     ...
     &                          (1. - rnum1) + rch_bactlp(jrch) * rchwtr
      ...
      totbactp = totbactp * Exp(-Theta(0.0,1.07,wtmp)*tday)
      totbactp = Max(0., totbactp)
      totbactlp = totbactlp * Exp(-Theta(0.0,1.07,wtmp)*tday) 
      totbactlp = Max(0., totbactlp)
