...
!! initialize variables
      ...
      qin = qday * pot_fr(j)   !inflow = surface flow    Ӷ       
      qdayi = qday
...
!       the remainder (1-pot_fr) goes directly to runoff
        pot_vol(j) = pot_vol(j) + qin! ݵص   ˮ  (mm)
        potflwi(j) = potflwi(j) + qin!    Ӷ       

!       compute surface area assuming a cone shape (m^2)
        potvol_m3 = pot_vol(j) * cnv!! ݵص   ˮ  (m3)
        potsa(j) = pi * (3. * potvol_m3 / (pi * hru_slp(j)))**.6666!  ˮ ı    
...
!      flow, evap, seepage, and redistribute soil water   û                   ʧ                               ·       ˮ
...
!       limit seepage into soil if profile is near field capacity      Ϊ          ʱ        ʿ      
         if (pot_k(j) > 0.) then
           yy = pot_k(j)
         else
           yy = sol_k(1,j)
